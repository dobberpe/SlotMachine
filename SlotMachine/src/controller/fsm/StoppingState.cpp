...
std::unique_ptr<IState> StoppingState::buttonPressed(SlotMachine& machine, SMWindow& window) {
	return nullptr;
}

std::unique_ptr<IState> StoppingState::update(SlotMachine& machine, SMWindow& window) {
	if (machine.spin()) {
